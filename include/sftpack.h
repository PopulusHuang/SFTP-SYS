 ...
 *       Filename:  sftfile.h
 *
 *    Description:  Define the SFTP-SYS's package protocol, 
 *
 *        Version:  1.0
 *           Date:  04/12/14 08:35:55 PM
 *       Compiler:  gcc-4.6.3
 *
...
typedef union sftdata{
	ACCOUNT user;			/* user name and passwd */	
	FILE_ATTR file_attr;	/* file attribute */
}SFT_DATA;

/* SFTP-SYS package structure */
typedef struct sft_pack{
		unsigned int order;			/* client order */	
		unsigned int ack;			/* server respond */
		char buf[DATA_SIZE];		/* meta data */	
		SFT_DATA data;
}SFT_PACK;
